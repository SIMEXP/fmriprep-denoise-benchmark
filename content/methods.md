...
- `std_dvars_threshold`: set the head motion cut-off value determined by the standard deviation of root mean square approach {cite:p}`power_scrubbing_2012` {cite:p}`jenkinson_2002`.
...
For the ICA-based approach, fMRIPrep implemented ICA-AROMA {cite:p}`pruim_ICAAROMA_2015`. 
