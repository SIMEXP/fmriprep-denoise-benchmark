...
SLURM_JOB_DIR = ".slurm"
SLURM_LOGS_DIR = ".logs"
FMRIPREP_SPECIFIER_PATTERN = (
    r"sub-[A-Za-z0-9]*_(ses-[A-Za-z0-9]*_)?([A-Za-z0-9_-]*)_space"
)
...
def create_slurm_scripts(args):
    subjects = list(
        p.name.split("sub-")[1]
        for p in Path(args.fmriprep_output).glob("sub-*/")
        if p.is_dir()
    )
    ...
    for atlas_type in ATLAS_COLLECTIONS:
        ...
        script_path = (
            Path(timeseires_output) / SLURM_JOB_DIR / f"{job_spec['jobname']}.sh"
        )
        header = [slurm_preamble.format(**job_spec), slurm_task_array]
        cmd_atlas = create_cmd_inputs(
            args, "${subject}", dataset, atlas_type, timeseires_output
        )
    ...
    print("Run jobs:")
    print(
        f'find {timeseires_output}/{SLURM_JOB_DIR}/timeseries_{dataset}_{fmriprep_version}_*.sh -type f | while read file; do sbatch "$file"; done'
    )
...
def parse_args():
    ...
    parser.add_argument("scratch_path", help="Path to scratch space.")
    parser.add_argument("fmriprep_output", help="Path to fMRIPrep output directory.")
    parser.add_argument(
        "participants_tsv",
        help="Path to participants.tsv in the original BIDS dataset.",
    )
    parser.add_argument(
        "virtualenv", help="Path to virtual environment of this project."
    ),
    parser.add_argument(
        "--slurm-account",
        action="store",
