...
- Additionally, any ROI that was imputed (tracked via a global imputation counts CSV)
  is plotted as a blue dot (with intensity based on its imputation count).
  ROIs assigned to the imputed group will not be plotted in the green/red groups.
...
def main():
    ...
    # Split data into imputed and non-imputed groups.
    ...
    # For non-imputed ROIs, further split into included (green) and excluded (red).
    ...
    # Plot imputed ROIs (blue), modulating intensity by imputation count.
    if not imputed_df.empty:
        ...
        for _, row in imputed_df.iterrows():
            ...
            # Use the Blues colormap: normalized value maps to a blue color.
