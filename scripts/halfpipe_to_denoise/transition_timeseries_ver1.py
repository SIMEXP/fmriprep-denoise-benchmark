...
This script:
  ...
  3. Converts the halfpipe feature name into the pipeline's naming by replacing "feature-"
     with "desc-".
  ...
  6. Creates a subject-specific folder in the output directory and writes the final file
     ...
     sub-pixar001_task-pixar_space-MNI152NLin2009cAsym_atlas-Schaefer2018Combined_nroi-434_desc-corrMatrixCompCor_timeseries.tsv
"""
...
def parse_args():
    ...
    # We force detrend and standardize to True by default
    parser.add_argument("--detrend", action="store_true", default=True,
                        help="Detrend the time series (default: True).")
    parser.add_argument("--standardize", action="store_true", default=True,
                        help="Standardize (z-score) the time series (default: True).")
...
def parse_filename(filename):
    ...
    Returns:
      subject: e.g., "sub-pixar001"
      feature: e.g., "corrMatrixCompCor" (obtained by removing "feature-")
      atlas: e.g., "Schaefer2018Combined" (obtained by removing "atlas-")
    """
    subject = None
    feature = None
    atlas = None
    ...
    for part in parts:
        if part.startswith("sub-"):
            subject = part  # e.g., "sub-pixar001"
        elif part.startswith("feature-"):
            feature = part.replace("feature-", "")  # e.g., "corrMatrixCompCor"
        elif part.startswith("atlas-"):
            atlas = part.replace("atlas-", "")      # e.g., "Schaefer2018Combined"
...
def clean_timeseries(df):
    """Detrend and standardize the timeseries using nilearn.signal.clean.
       This function forces detrend=True and standardize=True."""
    logging.info("Cleaning timeseries: detrend=True, standardize=True")
    arr = df.values.astype(float)
    # Force detrending and standardization
...
def main():
    ...
    # Recursively find TSV files ending with "_timeseries.tsv"
    ...
    for file_path in ts_files:
        ...
        # Only process files from within a task folder (e.g., containing "task-" in the path)
        if "task-" not in str(file_path.parent):
            logging.info("Skipping file not in a task folder: %s", fname)
            continue
        
        # Skip correlation/covariance matrices if they contain those keywords
        ...
        # Use halfpipe names directly:
        # Convert "feature-<name>" into "desc-<name>"
        desc_value = feature  # e.g., "corrMatrixCompCor"
        pipeline_atlas = atlas  # Use atlas name as is
        pipeline_nroi = args.nroi
        ...
        df = check_orientation_and_add_header(df, pipeline_nroi)
        df_clean = clean_timeseries(df)

        # Build final filename using a BIDS-like pattern:
        # Example:
        # sub-pixar001_task-pixar_space-MNI152NLin2009cAsym_atlas-Schaefer2018Combined_nroi-434_desc-corrMatrixCompCor_timeseries.tsv
        final_name = (
            f"{subject}_task-{args.task}_space-{args.space}_"
            f"atlas-{pipeline_atlas}_nroi-{pipeline_nroi}_"
            f"desc-{desc_value}_timeseries.tsv"
        )
        
        # Create a subject-specific folder inside the output directory
