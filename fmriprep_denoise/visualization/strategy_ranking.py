...
# strategy_order = list(utils.GRID_LOCATION_dof.values())
# fmriprep_versions = ["fmriprep-25.0.0"]
# datasets = ["ds000228"]


# def load_data(path_root, datasets, criteria_name="minimal"):
#     mean_total_ranking = []
...
def load_data(path_root, datasets, criteria_name="minimal"):
    ...
    # Ensure ranking is numeric by converting the column
    mean_total_ranking["ranking"] = pd.to_numeric(mean_total_ranking["ranking"], errors="coerce")
    
    return pd.pivot_table(
        mean_total_ranking,
        columns="strategy",
        index=["dataset", "version", "metrics"],
        fill_value=0,  # Changed from "ranking" to 0 to ensure numeric arithmetic
    )
...
def plot_ranking(data):
    ...
    for j, d in enumerate(datasets):
        for i, v in enumerate(fmriprep_versions):
            ...
            mat = mat.xs(v, level="version", drop_level=True)
            # mat = mat.droplevel(None, axis=1)
            mat = mat.droplevel("measure", axis=1)
            mat = mat.loc[
                ["modularity", "distance", "median", "p_values", "loss_df"],
                strategy_order
            ]
            # Ensure all values in mat are numeric
